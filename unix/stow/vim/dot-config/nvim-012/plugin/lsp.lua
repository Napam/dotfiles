...
require("lazyload").on_vim_enter(function()
  ...
  -- Codelens is opt-in per-buffer; enable globally so attached clients render it.
  -- Toggle via <leader>ul; lua_ls "0 References" can be disabled per-buf if noisy.
  vim.lsp.codelens.enable(true)

  -- WARN: per-client guard — LspAttach fires for every (client, buf) pair, but
  -- workspace_diagnostics is workspace-scoped; running it per buffer re-scans.
    ...
    callback = function(args)
      ...
      if client then
        if client:supports_method("textDocument/foldingRange", buf) then
          -- WARN: async attach — buffer may be shown in 0+ windows, none of
          -- them necessarily the current one. Set foldexpr on every window
        ...
        -- Workspace diagnostics. Two paths:
        --   1. Server supports `workspace/diagnostic` (LSP 3.17+ pull): use core API.
        --   2. Otherwise simulate via workspace-diagnostics.nvim (sends didOpen for
        --      every workspace file — expensive, run once per client).
        ...
        -- onTypeFormatting disabled — conform handles formatting; this yields surprising results.
        -- if client:supports_method("textDocument/onTypeFormatting", buf) then
        --   vim.lsp.on_type_formatting.enable(true, { bufnr = buf })
        -- end
      end

      -- Generic LSP keymaps — register once per buffer (LspAttach fires per client).
      -- (gd/gD/gr/gI/gy live in plugin/whichkey.lua via snacks picker.)
