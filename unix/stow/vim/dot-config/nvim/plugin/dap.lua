  ...
  for line in f:lines() do
    if not line:match("^%s*#") and not line:match("^%s*$") then
      ...
      if key and val then
        ...
        if quoted then
          -- WARN: quoted — `#` inside is part of the value, do NOT strip.
          val = quoted
        else
...
local function setup_dap_go(dap)
  dap.adapters.go = function(callback, config)
    ...
    if config.useKitty then
      -- HACK: delve doesn't pipe output to nvim; route via kitty for separate window.
      command = "kitty"
  ...
  dap.configurations.go = {
    ...
    },
    {
      ...
      program = "${fileDirname}",
      args = function()
        -- WARN: stable vim.treesitter API; old nvim-treesitter.ts_utils path
        -- is removed on `main` branch. Requires the go parser.
