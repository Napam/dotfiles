...
7. Delegate only to `low`, `med`, `deep` (via Agent tool, subagent_type set to one of those three). Never use other subagent types for tasks these tiers cover.
...
| User says                                                              | Action                              |
| ------------------------------------------------------------------------- | ------------------------------------ |
| "use low", "send to low", "let low handle", "tell low"                 | Agent tool, subagent_type: low, immediately  |
| "use med", "send to med", "let med handle", "tell med"                 | Agent tool, subagent_type: med, immediately  |
| "use deep", "send to deep", "go deep", "let deep handle", "tell deep"  | Agent tool, subagent_type: deep, immediately |
| "think deeply", "escalate", "this is hard"                             | Agent tool, subagent_type: deep, immediately |
| User gives file paths + line numbers + clear task                      | Agent tool, subagent_type: med, immediately  |

Hand-off prompt: `## Task` (user message verbatim) + `## Context` (conversation history subagent needs). Nothing else.
...
One Agent tool call (subagent_type: low | med | deep). Copy-paste skeleton for the prompt:
...
On re-delegation to same subagent, resume it with SendMessage (to: that agent's id) instead of spawning new, and insert before `## Constraints`:
