...
alias editutils='vim $HOME/dotfiles/unix/shellutils.sh && source $HOME/dotfiles/unix/shellutils.sh'
alias editenv='vim $HOME/dotfiles/unix/shellenv.sh && source $HOME/dotfiles/unix/shellenv.sh'
...
alias gp='git pull'
alias fgd='glol --color=always | fzf --ansi --reverse | xargs sh -c "git diff  \$0~1 \$0"'
...
venv () {
  if [ $# -lt 1 ]; then
    ...
    echo "Example: venv ml"
    return 1
  fi

  local venvDir=$HOME/.venvs/$1

  if [ ! -d $venvDir ]; then
    echo "$venvDir does not exist"
    return 1
  fi

  local venvActivate=$venvDir/bin/activate

  if [ ! -f $venvActivate ]; then
    local venvCreate=${venvDir}/create.sh
    echo "Missing $venvActivate, will run $venvCreate"
    bash $venvCreate
  fi
...
venvs () {
  local venvDir=$HOME/.venvs
  ls -1 --color=none $venvDir
}

venvclean () {
  if [ $# -lt 1 ]; then
    echo "Insufficient arguments"
    echo "Example: venvclean ml"
    return 1
  fi
  ...
  git -C $venvDir clean -fxd
}

venvupdate () {
  if [ $# -lt 1 ]; then
    echo "Insufficient arguments"
    echo "Example: venvupdate ml"
    return 1
  fi
